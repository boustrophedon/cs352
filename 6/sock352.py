...
class Packet:
    ...
    @staticmethod
    def data(buf, seq, nacl_box):
        encrypted = encrypt_message(buf, nacl_box)
    ...
    @staticmethod
    def from_bytes(buf, nacl_box):
        pkt = Packet()
        pkt.unpack(buf)
        if pkt.is_data():
            decrypted = decrypt_message(pkt.data, nacl_box)
            pkt.len = len(decrypted)
            pkt.data = decrypted
...
class Socket:
    ...
    def set_keys(self):
        my_priv_key = self.privateKeys[("*", "*")]

        addr, port = self.remote_addr
        pub_key = self.publicKeys.get((addr, str(port)))
        if pub_key is None:
            pub_key = self.publicKeys.get(("*",port))
        if pub_key is None:
            pub_key = self.publicKeys.get((addr,"*"))
        if pub_key is None:
            pub_key = self.publicKeys.get(("*","*"))

        if pub_key is None:
            raise Exception("remote pub key not found")

        self.nacl_box = Box(my_priv_key, pub_key)
    ...
    def connect(self,address):
        ...
        synrcv = Packet.from_bytes(buf, self.nacl_box)
    ...
    def accept(self):
        ...
        synsent = Packet.from_bytes(buf, self.nacl_box)
    ...
    def sendto(self,buffer):
        if self.state == STATE_ESTABLISHED:
            pkt = Packet.data(buffer, self.next_seq_send, self.nacl_box)
    ...
    def do_recv(self):
        ...
        try:
            buf, addr = self.sock.recvfrom(MAX_PKT)
            pkt = Packet.from_bytes(buf, self.nacl_box)
            if pkt.is_ack():
